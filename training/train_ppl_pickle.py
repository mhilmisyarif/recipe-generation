...
dir_path = 'D:\SKIRPSI\Project\indo-recipe-generation'

CHECKPOINT_DIR = dir_path + '\checkpoint'
...
def main():
    ...
    if args.trains:
        ...
        task = Task.init(project_name="SkripsiProject", task_name=args.trains)
        params = vars(args)
        params = task.connect(params)
         
    enc = encoder.get_encoder(args.model_name)
    hparams = model.default_hparams()
    with open(os.path.join(dir_path + '/models', args.model_name, 'hparams.json')) as f:
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.dataset == 'eval_only':
            ckpt = tf.train.latest_checkpoint(os.path.join(dir_path+'/models', args.model_name))
            ...
            if ckpt is None:
                ...
                ckpt = tf.train.latest_checkpoint(
                    os.path.join(dir_path+'/models', args.model_name))
